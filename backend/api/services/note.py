...
def create_note(note: NoteCreate, db: Session = Depends(get_db)):
    ...
    db.commit()
    db.refresh(db_note)
    return {"uuid": str(db_note.uuid), "name": db_note.name, "description": db_note.description, "finalized": db_note.locked}
...
def get_note_detail(uuid: str, db: Session = Depends(get_db)):
    ...
    return {"uuid": str(note.uuid), "name": note.name, "description": note.description, "finalized": note.locked}
